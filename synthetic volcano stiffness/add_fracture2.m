function C2=add_fracture2(lambda,G,e,theta,theta2)
...
M2=bond_transform(theta2,nx,ny,nz);
for i=1:nx
    for j=1:ny
        for k=1:nz
            Sf(:,:,i,j,k)=M(:,:,i,j,k)*Sf0(:,:,i,j,k)*M(:,:,i,j,k)'+M2(:,:,i,j,k)*Sf0(:,:,i,j,k)*M2(:,:,i,j,k)';
        end
    end
end
%%
