...
ylabel('t [s]');
title('v3 [m/s]');
print(gcf,[path 'seismogram.png'],'-dpng','-r200');
%% plot seismogram at the center
figure;
ax=plot(dt:dt:dt*nt,R1(17,:),'color','red');
hold on;
ax2=plot(dt:dt:dt*nt,R2(17,:),'color','green');
hold on;
ax3=plot(dt:dt:dt*nt,R3(17,:),'color','blue');
xlabel('t [s]');
ylabel('v [m/s]');
legend([ax,ax2,ax3],...
    '1','2','3',...
    'Location','best','orientation','vertical');
print(gcf,[path 'rec_center.png'],'-dpng','-r200');
%% make gif
sources=[path '/pic/'];
delaytime=.2;
filename='animation';
